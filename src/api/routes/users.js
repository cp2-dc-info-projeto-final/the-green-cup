...
router.get('/:id', verifyToken, isAdmin, async function(req, res) {
  try {
    const { id } = req.params;
    const result = await pool.query('SELECT id, nome, senha, email, role FROM usuario WHERE id = $1', [id]);
...
router.post('/', verifyToken, isAdmin, async function(req, res) {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO usuario (nome, email, senha, role) VALUES ($1, $2, $3, $4) RETURNING id, nome, email, role',
...
router.post('/login', async function(req, res) {
  try {
    ...
    const result = await pool.query(`SELECT 
      id, nome, email, senha as passwordHash, role
      FROM usuario 
      WHERE nome = $1`, [nome]);
...
router.put('/:id', verifyToken, isAdmin, async function(req, res) {
  try {
    ...
    if (existingUser.rows.length > 0) {
      ...
      return res.status(409).json({
        success: false,
        message: 'Nome já está em uso por outro usuário'
      });
    }
    ...
    if (senha && senha.trim() !== '') {
      ...
      const hashedPassword = await bcrypt.hash(senha, 12);
      query = 'UPDATE usuario SET nome = $1, email = $2, senha = $3, role = $4 WHERE id = $5 RETURNING id, nome, email, role';
